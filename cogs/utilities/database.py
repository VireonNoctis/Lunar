...
class MemoryResult:
    """
    Minimal stand-in for a Cassandra ResultSet used when
    running in memory mode (no ScyllaDB available).

    Repositories call .one()/.first()/.all() on results;
    in memory mode these simply return None / empty.
    """
...
class MemoryStatement:
    """Dummy prepared statement used in memory mode."""
...
class ScyllaDatabase:
    ...
    async def initialize(self):
        try:
            await self._initialize_scylla()
        except Exception as exc:
            # Graceful fallback: run in memory mode when ScyllaDB
            # is unavailable so the bot can still start.
            log.warning(
                "ScyllaDB unavailable (%s); "
                "starting in MEMORY mode. Data will not persist "
                "and account/database features are degraded.",
                exc,
            )
    ...
    async def _initialize_scylla(self):
    ...
    async def status(self) -> dict[str, Any]:
        ...
        if self._memory_mode:
            status["memory_mode"] = True
            status["healthy"] = False
            status["error"] = "Running in memory mode (ScyllaDB unavailable)"
            return status
...
class LeaderboardRepository(BaseRepository):
    ...
    async def top(
        ...
        *,
        ...
        Returns rows with snowflake_id, level, xp, username,
        display_name. In memory mode this returns [].
        """
    ...
    async def rank(
        ...
        """
        Rank of a single user by XP.

        Returns None in memory mode or when the user
        has no XP record.
        """
